...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    public void sendChatMessage(String msg, boolean addToChat)
    {
        ...
        if(msg.startsWith(CommandManager.COMMAND_PREFIX + "crash")){
            CrashManager.getManager().handleMethod(msg);
        } else if (msg.startsWith(CommandManager.COMMAND_PREFIX)) {
            CommandManager.getManager().handleCommand(msg);
        } else {
    ...
    public void drawBackground(int tint) {
        ...
        worldrenderer.pos(width, 0.0D, 0.0D).tex(1.0D, 1 + tint).color(64, 64, 64, 255).endVertex();
        worldrenderer.pos(0.0D, 0.0D, 0.0D).tex(1.0D, 1 + tint).color(64, 64, 64, 255).endVertex();
        tessellator.draw();
